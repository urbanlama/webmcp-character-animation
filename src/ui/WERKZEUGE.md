...
14. `trace` — „Zeigt den ABLAUF der Bewegung als Folge von drei GROSSEN Einzelbildern in einer Antwort — gleichmäßig über die Timeline verteilt, in derselben Kamera, wie ein Daumenkino zu lesen. Damit siehst du, ob eine Bewegung fließt, ohne jeden Frame einzeln anzufordern. Willst du einen Abschnitt genauer sehen, gib von und bis an — dann liegen die drei Bilder enger beieinander. Kamera wie bei look. RUFE DAS AUF, BEVOR DU EINE BEWEGUNG ABGIBST: ein einzelner Frame sagt nichts darüber, wie die Bewegung läuft.“ Parameter: `{von?: int, bis?: int, richtung_grad?: number, hoehe_grad?: number, ziel?: string, weite?: string}`.
