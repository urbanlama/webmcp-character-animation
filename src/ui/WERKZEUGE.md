...
14. `trace` — „Zeigt den VERLAUF der ganzen Bewegung in EINEM Bild: die Bahnen von Händen, Füßen und Becken über alle Frames, dazu die Figur an einem Frame deiner Wahl. Die Bahn zeigt die Form der Bewegung, der Abstand der Punkte das Timing — eng ist langsam, weit ist schnell, ein Knäuel ist Stillstand. Ein Knick in der Bahn ist ein Richtungswechsel. Damit siehst du, ob eine Bewegung fließt, ohne Frame für Frame durchzugehen. Kamera wie bei look. RUFE DAS AUF, BEVOR DU EINE BEWEGUNG ABGIBST: ein einzelner Frame sagt nichts darüber, wie die Bewegung läuft.“ Parameter: `{frame?: int, richtung_grad?: number, hoehe_grad?: number}`.
15. `look` — „Fotografiert EINEN Frame und liefert EIN grosses Bild, annotiert mit Bodengitter, Höhenleiste, Schwerpunkt, Kontaktpunkten und Stützfläche. Du richtest die Kamera selbst: Blickrichtung in Grad (0 von vorn, 90 von links), Blickhöhe in Grad (0 Augenhöhe, 90 von oben), worauf sie zielt (ein Gelenk aus describe_rig oder „figur") und wie nah sie herangeht (ganz/halb/nah, Anteile der gemessenen Körperhöhe). Ohne Angabe steht sie schräg von vorn und zeigt die ganze Figur. DEN VERLAUF EINER BEWEGUNG SIEHST DU, INDEM DU MEHRMALS AUFRUFST — je Frame ein Bild; bei gleicher weite ist der Maßstab derselbe und das Bodengitter steht fest, die Bilder sind vergleichbar. Ansinnen wie „zeig mir von vorne, wie das aussieht" oder „zeig mir das linke Knie groß" gehören hierher. Prüft nichts und beanstandet nichts; zum Prüfen dient validate. Frames ganzzahlig im Timeline-Bezug." Parameter: `{frame: int, richtung_grad?: number, hoehe_grad?: number, ziel?: string, weite?: string}`.
