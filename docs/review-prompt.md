Ich baue einen Beitrag für OpenAIs WebMCP Challenge. Gebaut wird von mehreren Coding-Agents, die parallel arbeiten. Das Ergebnis wird exakt so gut wie der Plan.

Unten steht der fertige Design-Plan. Er ist nicht aus der Luft gegriffen: Zwei Vorabtests wurden bereits durchgeführt und gemessen, ihre Ergebnisse stehen im Dokument und haben die Architektur bestimmt. Insbesondere ist ein erster Versuch, einen Agenten per Keyframe-Werkzeugen und Messfeedback einen Rückwärtssalto bauen zu lassen, gescheitert — mit dem stärksten verfügbaren Modell.

Ich will keine Zustimmung. Ich will, dass du den Plan zerlegst.

Prüfe ihn gezielt auf:

1. WIDERSPRÜCHE. Wo sagt der Plan an einer Stelle etwas anderes als an einer anderen?
2. UNTERSPEZIFIKATION. Wo würden zwei Coding-Agents dasselbe unterschiedlich bauen, weil der Plan es offen lässt? Nenne die konkreten Stellen.
3. BLINDE FLECKEN. Was übersieht der Plan, das ihn scheitern lässt? Besonders: Was zeigt sich erst spät?
4. LÜCKEN. Was fehlt vollständig und wird gebraucht?
5. BALLAST. Was steht drin, das nichts trägt oder das Vorhaben unnötig aufbläht?
6. REIHENFOLGE. Stimmt die Abhängigkeitskette? Was muss vor was, und wo ist die Reihenfolge im Plan falsch oder ungesagt?
7. ABNAHME. Reichen die Abnahmekriterien aus, um zu erkennen, dass ein Teil wirklich fertig UND richtig ist? Welches Kriterium ist zu schwach, welches fehlt?
8. TRAGFÄHIGKEIT. Wenn dieser Plan so umgesetzt wird — steht am Ende etwas, das funktioniert? Wenn nein, an welcher Stelle bricht es?

Konkrete Stellen nennen, nicht allgemein bleiben. Wenn du eine Aussage im Plan für falsch hältst, sag welche und warum.

Schreib nichts über Zeitbedarf, Aufwandsschätzung oder Terminplanung. Diese Frage steht hier nicht zur Debatte und ich habe dir die dafür nötigen Angaben auch nicht gegeben. Bewerte den Plan rein inhaltlich.

Sei hart, Höflichkeit hilft mir nicht.

--- BEGINN DES PLANS ---

# Design: Agentengesteuerte Charakteranimation im Browser

Beitrag zur OpenAI WebMCP Challenge.
...
und das schlägt direkt auf das Bewertungskriterium Nützlichkeit.
...
**Der Salto überzeugt am Ende nicht.** Rückfallposition: eine einfachere Bewegung
wird zur Demo — Sprung mit Landung, Fußballschuss — und der Salto als offener Punkt
benannt.
